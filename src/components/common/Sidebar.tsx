...
export default function Sidebar({ isCollapsed, toggleSidebar, isMobileOpen, closeMobileMenu }: SidebarProps) {
  ...
  const visibleNavItems = navItems.filter(item => {
    ...
    if (item.permission && user?.role !== 'admin') {
       // Check if permission exists in user.permissions map
       // Assuming user.permissions is Record<string, any>
       if (!user?.permissions || user.permissions[item.permission] === undefined) {
         return false;
       }
    }
    return true;
