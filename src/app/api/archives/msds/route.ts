// src/app/api/archives/msds/route.ts
import { NextRequest, NextResponse } from 'next/server';
import { prisma } from '@/lib/prisma';
import { logApiOperation, requirePermission } from '@/middleware/auth';
...
// GET: 获取MSDS文件列表（支持分页和搜索）
export const GET = async (req: NextRequest) => {
    const permResult = await requirePermission(req, 'archives', 'msds_view');
...
export const POST = async (req: NextRequest) => {
    const permResult = await requirePermission(req, 'archives', 'msds_upload');
    if (permResult instanceof NextResponse) return permResult;
    ...
    try {
        const archiveFile = await uploadArchiveFile({
            ...
            objectNamePrefix: 'archives/msds'
        });
        ...
        console.error('上传MSDS文件失败:', error);
        return NextResponse.json(
            { error: '上传失败，请重试' },
            { status: 500 }
        );
