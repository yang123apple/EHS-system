...
export default function EditTaskPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    Promise.all([
      apiFetch('/api/training/materials').then(res => res.json()),
      apiFetch(`/api/training/tasks/${id}`).then(res => res.json()),
      apiFetch('/api/structure').then(res => res.json()).catch(() => []), // 获取部门列表
      apiFetch('/api/users').then(res => res.json()).catch(() => []) // 获取用户列表
    ]).then(([materialsData, taskData, departmentsData, usersData]) => {
      console.log('[EditTaskPage] Loaded data:', { taskData, materialsData });
      ...
      if (taskData && taskData.task) {
        ...
        setMaterialId(task.materialId || '');
        
        // 处理日期格式
        if (task.startDate) {
          const startDate = new Date(task.startDate);
          const startDateStr = startDate.toISOString().split('T')[0];
          console.log('[EditTaskPage] Start date:', task.startDate, '->', startDateStr);
        ...
        if (task.endDate) {
          ...
          console.log('[EditTaskPage] End date:', task.endDate, '->', endDateStr);
        ...
        if (task.targetType === 'dept' && task.targetConfig) {
          try {
            const parsed = typeof task.targetConfig === 'string' 
              ? JSON.parse(task.targetConfig) 
              : task.targetConfig;
            ...
            console.error('Failed to parse targetConfig:', e);
          }
        } else if (task.targetType === 'user' && task.targetConfig) {
          try {
            const parsed = typeof task.targetConfig === 'string' 
              ? JSON.parse(task.targetConfig) 
              : task.targetConfig;
            ...
            if (Array.isArray(usersData)) {
              ...
              // 如果用户列表获取失败，从 assignments 中提取
              const userMap = new Map();
              task.assignments.forEach((a: any) => {
                if (a.user) {
                  userMap.set(a.userId, a.user.name);
                }
              });
              display = config
        ...
        setDisplayTargets(display);
      } else {
        console.error('[EditTaskPage] Task data not found in response:', taskData);
        alert('任务数据格式错误');
      }
    }).catch(error => {
      console.error('[EditTaskPage] Failed to load data:', error);
      alert('加载任务数据失败: ' + (error.message || String(error)));
    }).finally(() => {
      setLoading(false);
    });
  }, [id]);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
        const res = await apiFetch(`/api/training/tasks/${id}`, {
            method: 'PUT',
            body: {
                title,
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-slate-900">编辑学习任务</h1>
              <p className="text-sm text-slate-500 mt-1">修改学习计划信息</p>
            </div>
          </div>
        </div>
