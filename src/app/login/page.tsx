    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-hytzer-dark relative overflow-hidden z-10">
      ...
      <div className="bg-white/10 backdrop-blur-lg border border-white/20 p-6 sm:p-8 rounded-2xl shadow-2xl w-full max-w-md z-20 mx-4">
        <div className="text-center mb-6 sm:mb-8">
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-white">EHS 信息化管理平台</h2>
          <p className="text-slate-300 text-xs sm:text-sm mt-2">安全 · 环保 · 健康 · 高效</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
          ...
          <div>
            <label className="block text-xs sm:text-sm font-medium text-slate-300 mb-1">账号</label>
            <input
              type="text"
              value={username}
              onChange={(e) => setUsername(e.target.value)}
              className="w-full px-3 sm:px-4 py-2 bg-slate-800/50 border border-slate-600 rounded-lg text-white text-sm sm:text-base focus:ring-2 focus:ring-hytzer-blue focus:border-transparent outline-none transition-all placeholder-slate-500"
              placeholder="请输入用户名"
            />
          </div>

          <div>
            <label className="block text-xs sm:text-sm font-medium text-slate-300 mb-1">密码</label>
            <input
              type="password"
              value={password}
          ...
          <button
            type="submit"
            className="w-full bg-hytzer-blue hover:bg-blue-600 text-white font-bold py-2.5 sm:py-3 rounded-lg transition-all transform hover:scale-[1.02] shadow-lg shadow-blue-900/50 cursor-pointer text-sm sm:text-base"
          >
            登 录
          </button>
        </form>
