...
async function createTestUsers() {
  ...
  console.log(`✅ 创建完成：${users.length} 个测试用户`);
  return allUsers;
}
...
async function generateHazards(users: any[]) {
  ...
  for (let batch = startBatch; batch < batches; batch++) {
    ...
    try {
      ...
      console.error(`❌ 批次 ${batch + 1} 插入失败:`, error);
      throw error;
    }
  }
...
async function verifyData() {
  ...
  const [hazardCount, visibilityCount, userCount] = await Promise.all([
    ...
    (prisma as any).hazardVisibility.count(),
    prisma.user.count({ where: { username: { startsWith: 'test_user_' } } }),
  ]);
