...
async function createTestUsers() {
  ...
  const newUsers = users.filter(u => !existingUsernames.has(u.username)).map(u => ({
    ...u,
    id: crypto.randomUUID() // 显式生成ID，避免 createMany 在某些数据库上的限制
  }));

  if (newUsers.length > 0) {
    console.log(`   需要创建 ${newUsers.length} 个新用户...`);
