...
async function initNotificationTemplates() {
  ...
  // 调试信息
  console.log('Prisma Client:', prisma ? '已加载' : '未加载');
  console.log('NotificationTemplate Model:', prisma.notificationTemplate ? '存在' : '不存在');
  
  if (!prisma.notificationTemplate) {
    console.error('❌ NotificationTemplate 模型不可用，请检查 Prisma 客户端生成');
    return;
  }
